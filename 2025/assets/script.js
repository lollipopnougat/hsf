...
const helpContent = {
  shellType: {
    header: '外形',
    body: '将发射的烟花类型。选择“随机”即可获得不错的表现'
  },
  shellSize: {
    header: '大小',
    body: '烟花的尺寸大小。根据真实的烟花弹尺寸建模，较大的尺寸不仅烟花炸开的半径更大，火花数量还会更多，有时效果也更复杂。请注意较大的尺寸也需要更多的计算，并且可能会导致延迟'
  },
  quality: {
    header: '质量',
    body: '整体图形质量。如果动画运行不流畅，请尝试降低质量，高品质会大大增加渲染的火花数量，并可能导致延迟'
  },
  skyLighting: {
    header: '天空照亮',
    body: '当烟花爆炸时照亮背景。如果屏幕上的背景看起来太亮，请尝试将其设置为“暗淡”或“无”'
  },
  scaleFactor: {
    header: '缩放',
    body: '允许缩放所有烟花的大小，本质上是模拟距离烟花的远近程度，在手机或平板电脑上选择较大的烟花尺寸时，缩小比例可能效果更好一些'
  },
  autoLaunch: {
    header: '自动燃放',
    body: '自动燃放一系列烟花，无需其他操作'
  },
  finaleMode: {
    header: '最终模式',
    body: '模拟烟花表演最后一端猛烈燃放的情景，可能会导致延迟，本功能在启用“自动燃放”后才有效'
  },
  hideControls: {
    header: '隐藏控制按钮',
    body: '隐藏屏幕顶部的半透明控件，隐藏后仍然可以点击右上角重新打开此菜单'
  },
  fullscreen: {
    header: '全屏',
    body: '切换全屏模式'
...
function init() {
  ...
  shellNames.forEach((opt) => (options += `<option value="${opt}">${shellChsNames[opt]}</option>`));
...
const randomTitle = () => {
  const titles = ['神龙纳福，钱龙驾到', '龙舞灯笼照，幸福安康到', '龙行祥云端，福禄寿喜全', '龙年舞狮庆，欢乐幸福年', '龙腾剪纸间，好运永相连'];
  return titles[Math.floor(MyMath.random(0, titles.length - 1))];
};

window.onload = () => {
  setName();
  const subtitle = document.querySelector('.banner-sub-title3');
  subtitle.innerText = randomTitle();
  const banner = document.querySelector('.banner');
  const closeBanner = () => {
    banner.classList.add('mhide');
    setTimeout(() => {
      banner.classList.add('close');
    }, 800);
  };
  document.addEventListener('click', closeBanner);
  document.addEventListener('touchstart', closeBanner);
